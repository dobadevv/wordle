...
import { MAX_WORD_LENGTH, MIN_WORD_LENGTH, isValidWordLength, resolveGameStatus } from "./gameRules";
...
export default function WordlePage() {
    ...
    return <Flex className="min-h-screen" justify="center" align="center">
        <Card className="shadow-lg">
            <Flex vertical gap="middle">
                ...
                <Flex gap="small">
                    ...
                    <Button type="primary" onClick={startNewGame} disabled={loading}>New game</Button>
                </Flex>
                <WordleGrid wordLength={wordLength} rows={rows} />
                <Input value={guess} onChange={(e) => setGuess(e.target.value)} placeholder="Input your guess" maxLength={wordLength} disabled={isGameOver || loading} />
                <Button className="!w-full" type="primary" onClick={handleSubmit} loading={loading} disabled={isGameOver}>Submit</Button>
                {
                    isGameOver && (gameStatus === 'won'
                        ? <Alert type="success" title="You won" />
                        : <Alert type="error" title="You lose" />
                    )
                }
                {
